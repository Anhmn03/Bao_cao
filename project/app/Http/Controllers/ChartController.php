...
public function getEmployeeCountByDepartment($parent)
{
    $allDepartmentIds = $this->getAllDepartmentIds($parent->id);
    return User::whereIn('department_id', $allDepartmentIds)->count();
}

public function getAgeDistributionByDepartment($parent)
{
    $allDepartmentIds = $this->getAllDepartmentIds($parent->id);
    ...
    return $this->groupAges($ages);
}

public function getGenderDistributionByDepartment($parent)
{
    $allDepartmentIds = $this->getAllDepartmentIds($parent->id);
...
private function getEmployeeStats($parentDepartments, $isFilterAll = false)
{
    $labels = [];
    $employeeData = [];
    ...
    foreach ($parentDepartments as $parent) {
        if ($isFilterAll) {
            // Lấy tất cả phòng ban con của phòng ban cha
            $allDepartmentIds = $this->getAllDepartmentIds($parent->id);
            // Tổng số nhân sự của phòng ban cha và tất cả phòng ban con
            $employeeCount = User::whereIn('department_id', $allDepartmentIds)->count();
        } else {
            // Chỉ lấy số lượng nhân sự trực tiếp của phòng ban
            $employeeCount = $this->getDirectEmployeeCountByDepartment($parent->id);
        }

        $labels[] = $parent->name;
        $employeeData[] = $employeeCount;

        // Đệ quy xử lý các phòng ban con
        if (!$isFilterAll && !empty($parent->children)) {
            $childStats = $this->getEmployeeStats($parent->children, $isFilterAll);
            $labels = array_merge($labels, $childStats['labels']);
            $employeeData = array_merge($employeeData, $childStats['employeeData']);
        }
    }
...
private function getAgeStats($parentDepartments, $colors)
{
    $ageDatasets = [];

    foreach ($parentDepartments as $parent) {
        $ageGroups = $this->getAgeDistributionByDepartment($parent);
    ...
    return array_values($ageDatasets);
}
private function getGenderStats($parentDepartments)
{
    $genderDatasets = [
        'male' => [],
        'female' => [],
    ...
    foreach ($parentDepartments as $parent) {
        $genderData = $this->getGenderDistributionByDepartment($parent);
        $genderDatasets['male'][] = $genderData['male'];
        $genderDatasets['female'][] = $genderData['female'];
    }

    return $genderDatasets;
}




private function filterDepartments(Request $request)
{
    $selectedDepartmentId = $request->get('department_id');

    if ($selectedDepartmentId) {
        // Lọc theo phòng ban cụ thể
        $parentDepartments = Department::where('id', $selectedDepartmentId)->get();
        $isFilterAll = false;
    } else {
        // Lấy tất cả các phòng ban cha
        $parentDepartments = Department::where('parent_id', 0)->get();
        $isFilterAll = true;
    }

    return compact('isFilterAll', 'parentDepartments');
}

public function userDepChart(Request $request)
{
    // Lọc phòng ban dựa trên request
    $filterResult = $this->filterDepartments($request);
    $isFilterAll = $filterResult['isFilterAll'];
    $parentDepartments = $filterResult['parentDepartments'];

    // Màu sắc mặc định
    $colors = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF'];

    // Lấy dữ liệu thống kê
    $employeeStats = $this->getEmployeeStats($parentDepartments, $isFilterAll);
    $ageStats = $this->getAgeStats($parentDepartments, $colors);
    $genderStats = $this->getGenderStats($parentDepartments);
    $seniorityData = $this->categorizeEmployeesBySeniority();

    // Truyền danh sách phòng ban để hiển thị dropdown
    $departments = Department::all();

    return view('fe_chart.user_department', [
        'labels' => $employeeStats['labels'],
        'employeeData' => $employeeStats['employeeData'],
        'ageDatasets' => $ageStats,
        'genderDatasets' => $genderStats,
        'colors' => $colors,
...
// {
//     $departmentIds = Department::where('parent_id', $parentId)->pluck('id')->toArray();
//     foreach ($departmentIds as $id) {
//         $departmentIds = array_merge($departmentIds, $this->getAllDepartmentIds($id));
//     }

//     return array_merge([$parentId], $departmentIds); // Bao gồm cả phòng ban cha
// }

private function getAllDepartmentIds($parentId)
{
    $departmentIds = [];
    $queue = [$parentId]; // Khởi tạo queue với parentId

    while (!empty($queue)) {
        $currentId = array_shift($queue);
        $departmentIds[] = $currentId;

        // Thêm các phòng ban con vào queue
        $childIds = Department::where('parent_id', $currentId)->pluck('id')->toArray();
...
private function getDirectEmployeeCountByDepartment($departmentId)
{
